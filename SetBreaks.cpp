...
int main () {
    ...
    while (getline (source_file, source_line)) 
    {
        ...
        // find sym tag
            ...
            // save file position
            auto read_pos = source_file.tellg();
            // *********
            // get a new line until a label is found 
            // and not a comment or a tag (beginning with a comment )
            do {
                // get next line
                getline (source_file, source_line);
                pos = source_line.find_last_of("|")+2 ;
                pos2 = source_line.find_first_of(" ", pos);
                label = source_line.substr(pos,pos2-pos);
            ...
            // reset file position just after sym tag
            source_file.seekg(read_pos);          
            }
